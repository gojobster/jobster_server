...
public class CriptografiaSimetrica {
	...
	public byte[] encriptar (String cadenaOriginal) throws TalendorseException {
		...
		try {
			if((_key != null) && (_IV != null))
			{
				...
				throw new TalendorseException("Error al encryptar: Error al inicializar la clave y el vector");
			}
		} catch (Exception e) {
			throw new TalendorseException("Error al encryptar");
		}
	}
	public String desencriptar (byte[] cadenaOriginal) throws TalendorseException {
    ...
    public void cifrarDescifrarArchivo(String inFileName, String outFileName, CryptoAction action) throws TalendorseException {
        //si el archivo especificado no existe,
        if (!Files.exists(Paths.get(inFileName))) {
            // generamos una excepción informándolo.
            throw new TalendorseException("No se ha encontrado el archivo.", -33);
        }
        try {
            ...
            if (!_key.isEmpty() && !_IV.isEmpty()) {
                ...
                throw new TalendorseException("error llave no valida");
            }
        } catch (TalendorseException ex) {
            throw ex;
        } catch (Exception ex) {
            throw new TalendorseException("Error al Encriptar");
        }
    }
